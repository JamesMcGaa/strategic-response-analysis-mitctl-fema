...
def f_getLatLong(f_city, f_country):
  time.sleep(1.15) #AXR0131: Changed time from .15 to 1.15 as google api seems to return error for to many requests in a short time frame?
  f_city.strip()
  f_country.strip()
...
inputFilePath = 'C:/Users/alr06kc/Dropbox (MIT)/Humanitarian Response Capacity Stockpile/Alex/04 - Python Strategic Response Analysis/pythonCodeGetData/' #AXR0131:changed path
depotCitiesFileName = 'warehouseCitiesLookupOutput.csv'

depotCitiesData = f_myReadCsv(inputFilePath + depotCitiesFileName)
...
depotCities = column(depotCitiesData[1], [i for i in range(len(depotCitiesData[0])) if depotCitiesData[0][i].strip() == 'OutputCity'][0])
depotCountries = column(depotCitiesData[1], [i for i in range(len(depotCitiesData[0])) if depotCitiesData[0][i].strip() == 'OutputLocation'][0])

gglInfo = []

gglInfo.append(['depotCity', 'depotCountry', 'gglCountryAscii', 'gglAddressAscii', 'gglLat', 'gglLong'])

for i in range(len(depotCities)):
  gglInfo.append(deepcopy(f_getLatLong(depotCities[i], depotCountries[i])))

  


f_writeCsv(inputFilePath + 'depotGglLatLong.csv', gglInfo)
