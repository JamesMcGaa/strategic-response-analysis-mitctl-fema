...
def GetDistanceFct(f_origLat, f_origLong, f_destLat, f_destLong,i):
  ...
  url = "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins={0},{1}&destinations={2},{3}&key=AIzaSyDRoSUaq8qM9MsDQSNA4gSuEF8MhT8hoQs".format(str(f_origLat), str(f_origLong), str(f_destLat), str(f_destLong))
  ...
  if apiOutput['rows'][0]['elements'][0]['status'] == 'OK':
    ...
    drivingTime_hrs = 1e6
    distance_km = 1e6
  elif apiOutput['rows'][0]['elements'][0]['status'] == 'REQUEST_DENIED':
    print 'Your API key is out of date!'
